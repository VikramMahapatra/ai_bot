...
def get_session_conversations_report(
    ...
    metrics_subquery = (
        db.query(
            ...
            func.avg(ConversationMetrics.average_response_time).label(
        ...
        .filter(*metrics_filters)
        .group_by(ConversationMetrics.session_id)
        .subquery()
    )
    ...
    leads_subquery = (
        ...
        .outerjoin(
            LeadContactMapping, LeadContactMapping.contact_id == Conversation.contact_id
        )
        .outerjoin(Lead, Lead.id == LeadContactMapping.lead_id)
    ...
    query = (
        db.query(
            ...
            func.coalesce(metrics_subquery.c.completion_tokens, 0).label(
                "completion_tokens"
            ),
            func.coalesce(metrics_subquery.c.average_response_time, 0.0).label(
                "average_response_time"
            ),
    ...
    if min_tokens is not None:
        query = query.filter(
            func.coalesce(metrics_subquery.c.total_tokens, 0) >= min_tokens
        )
    if max_tokens is not None:
        query = query.filter(
            func.coalesce(metrics_subquery.c.total_tokens, 0) <= max_tokens
        )
    if has_lead is not None:
    ...
    if search:
        query = query.filter(
            func.lower(func.coalesce(contact_subquery.c.contact_name, "Guest")).ilike(
                f"%{search.lower()}%"
            )
        )
    ...
    funnel_categories = get_funnel_categories(
        db, organization_id, include_inactive=True
    )
        ...
        metrics.append(
            {
                ...
                "user_satisfaction": (
                    float(row.user_satisfaction)
                    if row.user_satisfaction is not None
                    else None
                ),
...
def get_report_summary(
    ...
    average_duration = (
        sum(m.conversation_duration for m in metrics) / total_conversations
    ...
    return {
        ...
        "average_satisfaction_rating": (
            round(average_satisfaction, 2) if average_satisfaction else None
        ),
        "plan_usage": plan_usage,
    }
...
def get_token_usage_report(
    ...
    average_tokens = (
        total_tokens / conversations_count if conversations_count > 0 else 0
    )
...
def get_leads_report(
    ...
    conversion_rate = (
        (total_leads / total_conversations * 100) if total_conversations > 0 else 0
    )
...
def get_voice_campaign_report(
    ...
    if start_date:
        sum_call_duration_query = sum_call_duration_query.filter(
            func.coalesce(CallLog.start_time, CallLog.created_at) >= start_date
        )

    if end_date:
        sum_call_duration_query = sum_call_duration_query.filter(
            func.coalesce(CallLog.start_time, CallLog.created_at) <= end_date
        )
    ...
    return {
        ...
        "summary": {
            ...
            "sum_call_duration_label": _format_duration_label(
            ...
            "campaign_duration_label": _format_duration_label(
...
def get_daily_conversation_stats(
    ...
    query = (
        ...
        .group_by(func.date(ConversationMetrics.conversation_start))
        .order_by(func.date(ConversationMetrics.conversation_start).desc())
    )
...
def sync_conversation_metrics(
    ...
    try:
        ...
        existing = (
            ...
            .filter(ConversationMetrics.conversation_id == conversation_id)
            .first()
        )
        ...
        lead = (
            ...
            .filter(LeadContactMapping.contact_id == conversation.contact_id)
            .first()
        )

        session_start, session_end, session_turns = (
            db.query(
        ...
        if session_start and session_end:
            conversation_duration = max(
                (session_end - session_start).total_seconds(), 0.0
            )
...
def sync_voice_metrics_from_conversation(
    ...
    try:
        ...
        lead = (
            db.query(Lead)
            .join(LeadContactMapping)
            .filter(LeadContactMapping.contact_id == conversations[0].contact_id)
            .first()
        )
