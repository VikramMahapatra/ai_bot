...
def _sync_lead_contact_to_agent_list(db: Session, widget_config: WidgetConfig, lead: LeadCreate) -> None:
...
async def create_lead(
    ...
    try:
        ...
        if existing is None or existing.funnel_stage not in {"closed_won", "closed_lost"}:
            
            contact = db.query(Contact).filter(Contact.session_id == lead.session_id).first()
