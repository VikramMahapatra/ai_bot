...
async def get_dashboard_stats(
    ...
    try:
        ...
        logger.info(
            f"Dashboard stats for org_id: {org_id}, user: {current_user.username}"
        )
        ...
        # Total widgets
        total_widgets = (
            db.query(func.count(WidgetConfig.id))
            .filter(WidgetConfig.organization_id == org_id)
            .scalar()
            or 0
        )
        ...
        # Total Agents
        total_agents = (
            db.query(func.count(CallingAgent.id))
            .filter(CallingAgent.organization_id == org_id)
            .scalar()
            or 0
        )
        ...
        pipeline_leads = (
            db.query(func.count(Lead.id))
            .filter(Lead.organization_id == org_id)
            .filter(Lead.funnel_stage == "lead_qualification")
            .scalar()
            or 0
        )
        ...
        # Pipeline Leads in last 7 days
        pipeline_leads_7d = (
            db.query(func.count(Lead.id))
            .filter(
                and_(
                    Lead.organization_id == org_id,
                    Lead.funnel_stage == "lead_qualification",
                    Lead.created_at >= seven_days_ago,
                )
            )
            .scalar()
        ...
        if total_conversations > 0:
            ...
            session_counts = (
                db.query(func.count(Conversation.id).label("count"))
                .filter(Conversation.organization_id == org_id)
            ...
            if session_counts:
                ...
                avg_messages_per_session = round(
                    total_messages / len(session_counts), 2
                )
        ...
        return {
            ...
            "pipeline_leads_7d": pipeline_leads_7d,
        }
...
async def get_daily_conversations(
    ...
    try:
        ...
        for date_str in sorted(date_counts.keys()):
            data.append({"date": date_str, "count": date_counts[date_str]})
...
async def get_leads_funnel(
    ...
    try:
        ...
        rows = (
            db.query(Lead.funnel_stage, func.count(Lead.id).label("count"))
            .filter(Lead.organization_id == org_id)
            ...
            data.append(
                {
                    ...
                    "conversion_rate": (
                        0.0
                        if top_count <= 0
                        else round((unassigned / top_count) * 100, 1)
                    ),
                }
            )
...
async def get_top_sessions(
    ...
    try:
        ...
        for session_id, msg_count, last_msg in top_sessions:
            ...
            lead = (
                db.query(Lead)
                .filter(Lead.session_id == session_id, Lead.organization_id == org_id)
                .first()
            )
