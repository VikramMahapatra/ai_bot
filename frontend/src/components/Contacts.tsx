...
const Contacts = ({ tab, setTab }: ContactsProps) => {
  ...
  const [contactListLookupItems, setContactListsLookupItems] = useState<
    ContactListItem[]
  >([]);
  ...
  const [editContactList, setEditContactList] =
  ...
  const saveContact = async () => {
    ...
    try {
      ...
      showError(
        err?.response?.data?.detail || err?.detail || "Failed to save the data",
      );
    } finally {
      setLoading(false);
    }
  ...
  const handleManualUpload = async () => {
    ...
    setLoading(true);
    try {
      const result = await campaignService.uploadContactsManual(
        Number(uploadListId),
        { contacts: manualContacts },
      );
      setManualContacts([]);
      showSuccess(
        `Manual upload complete: ${result.created} created, ${result.failed} failed`,
      );
      await loadContactLists();
      ...
      showError(
        err?.response?.data?.detail || "Failed to upload manual contacts",
      );
    } finally {
      setLoading(false);
    }
  ...
  const handleCsvUpload = async () => {
    ...
    try {
      ...
      const result = await campaignService.uploadContactsCsv(
        Number(uploadListId),
        formData,
      );
      setCsvFile(null);
  ...
  const handleDownloadCsvTemplate = () => {
    const template = `name,email,phone,whatsapp_number,gender,company,designation,item_name,item_type,interest_stage,item_category,amount,offer_value,city,state,country,source,lifecycle_stage,tags
                        ...
                        <TableCell>
                          <Stack direction="row" spacing={1}>
                            <Button
                              ...
                              onClick={() =>
                                handleSelectListForContacts(list.id)
                              }
                            >
                              View Contacts
                            </Button>
        ...
        <Stack spacing={2.5}>
          ...
          {/* <Paper sx={{ ...sectionPanelSx, p: 2.5 }}>
            ...
            >
              ...
              <Button
                size="small"
                sx={{ ...compactButtonSx, minWidth: 95 }}
                variant="contained"
          ...
          <Paper sx={{ ...sectionPanelSx, p: 2.5 }}>
              ...
              {showFormat && (
                <Box mt={1}>
                  <Typography variant="caption" display="block">
                    <strong>Basic:</strong> name, email, phone, whatsapp_number,
                    company, designation
                  </Typography>

                  <Typography variant="caption" display="block">
                    <strong>Product:</strong> item_name, item_type,
                    interest_stage, item_category
                  </Typography>

                  <Typography variant="caption" display="block">
                    <strong>Other:</strong> amount, offer_value, city, state,
                    country, source, lifecycle_stage, tags
                  </Typography>
                  ...
                  <Typography
                    ...
                    sx={{ mt: 0.5 }}
                  >
                    • At least one of <strong>name, email, phone </strong> is
                    required
                  </Typography>
                  <Typography
                    variant="caption"
            ...
            >
              ...
              <Button
                ...
                disabled={
                  !uploadCountryCode || !csvFile || !uploadListId || loading
                }
              >
                Upload File
              </Button>
                  ...
                  >
                    ...
                    <Box>
                      ...
                      <Typography variant="body2" sx={{ color: subTextColor }}>
                        ...
                        {uploadResult.updated} updated, {uploadResult.failed}{" "}
                        failed.
                      </Typography>
                    </Box>
                  </Box>
            ...
            >
              <TextField
                ...
                onChange={(e) =>
                  setForm({
                    ...form,
                    contact_list_id: Number(e.target.value) || null,
                  })
                }
