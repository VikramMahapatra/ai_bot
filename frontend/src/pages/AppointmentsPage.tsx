...
const AppointmentsPage: React.FC = () => {
  ...
  const [rescheduleMeetLink, setRescheduleMeetLink] = useState(
  ...
  const loadAppointments = async () => {
    try {
      ...
      setAppointmentTotal(data.pagination?.total || 0);
    } catch (err: any) {
  ...
  const loadMeetingSettings = async () => {
    try {
      ...
      const nextDefault =
        (meetingSettings?.default_meet_link || "").trim() || DEFAULT_MEET_LINK;
  ...
  const updateStatus = async (
    id: number,
    nextStatus: AppointmentItem["status"],
  ) => {
    try {
      setSuccess("");
  ...
  const saveDefaultMeetLink = async () => {
    ...
    try {
      ...
      setError("");
      const response =
        await organizationService.updateMeetingSettings(nextLink);
      const savedLink =
        (response?.default_meet_link || "").trim() || DEFAULT_MEET_LINK;
  ...
  const submitReschedule = async () => {
    ...
    try {
      ...
      if (response?.notification?.sent) {
        ...
        Array.isArray(response?.notification?.errors) &&
        response.notification.errors.length
      ) {
        setSuccess(
          "Appointment rescheduled, but some notification emails failed to send.",
        );
      } else {
      ...
      setError(
        err?.response?.data?.detail || "Failed to reschedule appointment",
      );
    } finally {
  ...
  const handleCalendarDrop = (dropDate: Date) => {
    ...
    const dragged = appointments.find(
      (item) => item.id === draggedAppointmentId,
    );
    setDraggedAppointmentId(null);
    if (!dragged) return;

    const nextDate = combineDateWithSourceTime(
      dropDate,
      dragged.appointment_at,
    );
  ...
  const calendarTitle = useMemo(() => {
    if (calendarView === "day") {
      return calendarCursor.toLocaleDateString([], {
        weekday: "long",
        month: "long",
        day: "numeric",
        year: "numeric",
          ...
          {tab === 1 && (
            <Stack spacing={2} sx={{ mt: 2 }}>
                    ...
                    return (
                      <Paper
                        ...
                        sx={{
                          ...
                          minHeight: 170,
                          borderRadius: 2,
                          borderColor: draggedAppointmentId
              ...
              {calendarView === "month" && (
                <Paper
                  variant="outlined"
                  sx={{ p: 1.2, borderRadius: 2.5, overflowX: "auto" }}
                >
                  <Box sx={{ minWidth: 840 }}>
                    <Box
                      sx={{
          ...
          <DialogContent sx={{ pt: 1.5 }}>
            <Stack spacing={2}>
              <Alert severity="info" variant="outlined">
                Reschedule email will be sent to the participant and admin
                escalation contacts (L1 and L2) with the Google Meet link.
              </Alert>
