...
const WidgetManagementPage: React.FC = () => {
  ...
  const [widgetTestLinks, setWidgetTestLinks] = useState<Record<string, WidgetTestLinkMeta>>({});
...
<script src="https://your-domain.com/widget/dist/chatbot-widget.iife.js"><\/script>`;
  ...
  const getAgentTestUrl = (widgetId: string, token: string) =>
    buildPublicUrl(`/agent-test/${encodeURIComponent(widgetId)}?token=${encodeURIComponent(token)}`);
  ...
  const fetchWidgetTestLink = useCallback(
    async (widgetId: string, options?: { extraHours?: number; silent?: boolean }): Promise<WidgetTestLinkMeta | null> => {
      ...
      try {
        const response = await api.get(`/api/admin/widget/test-link/${encodeURIComponent(widgetId)}`, {
        ...
        const nextMeta: WidgetTestLinkMeta = {
          ...
          startAt: typeof response?.data?.start_at === 'string' ? response.data.start_at : '',
          expiresAt: typeof response?.data?.expires_at === 'string' ? response.data.expires_at : '',
          loading: false,
          attempted: true,
        };
        ...
        setWidgetTestLinks((prev) => ({
          ...prev,
          [widgetId]: { ...(prev[widgetId] || {}), loading: false, attempted: true },
        }));

        if (!silent) {
          setError(err?.response?.data?.detail || 'Failed to generate expiring test URL');
        }
        return null;
      }
    },
  ...
  const handleExtendTestUrl = async (widgetId: string) => {
    ...
    setSuccess('Test link window reset to now and expiry set to next 24 hours.');
    setError('');
  };
  ...
  const handleOpenEmailShare = async (widget: WidgetConfig) => {
    ...
    setEmailShareSubject(`Welcome to Zentrixel - Test ${widget.name || 'AI Assistant'}`);
  ...
  const handleSendEmailShare = async () => {
    ...
    if (!emailShareWidgetId) {
      setError('Widget context missing for email sharing. Please reopen the dialog.');
      return;
    }
  ...
  useEffect(() => {
    ...
    widgets.forEach((widget) => {
      const widgetId = widget.widget_id?.trim() || '';
      if (!widgetId || widgetTestLinks[widgetId]?.attempted || widgetTestLinks[widgetId]?.loading) {
        return;
      }
  ...
  return (
    ...
    <Box>
      <Stack spacing={3}>
        ...
        >
          <Stack direction={{ xs: 'column', sm: 'row' }} spacing={1} sx={{ px: 2, py: 1.3, borderBottom: `1px solid ${alpha(theme.palette.primary.main, 0.14)}` }}>
            <Chip size="small" color="primary" label={`Total Agents: ${linkStatusSummary.total}`} />
            <Chip size="small" color="success" variant="outlined" label={`Active Links: ${linkStatusSummary.active}`} />
            <Chip size="small" color="error" variant="outlined" label={`Expired Links: ${linkStatusSummary.expired}`} />
          </Stack>
          ...
          <TableContainer sx={{ overflowX: 'auto' }}>
            ...
            >
              <TableHead sx={{ backgroundColor: alpha(theme.palette.primary.main, 0.09) }}>
                <TableRow>
                  <TableCell sx={{ fontWeight: 700, width: '22%' }}>Agent Name</TableCell>
                  <TableCell sx={{ fontWeight: 700, width: '16%' }}>Lead Capture</TableCell>
                  <TableCell sx={{ fontWeight: 700, width: '34%' }}>Test Link Expiry</TableCell>
                  <TableCell sx={{ fontWeight: 700, width: '12%' }}>Created</TableCell>
                  <TableCell sx={{ fontWeight: 700, textAlign: 'right', width: '16%' }}>Actions</TableCell>
                </TableRow>
              </TableHead>
              <TableBody>
                {widgets.length === 0 ? (
                  ...
                  widgets.map((widget, index) => {
                    const widgetId = widget.widget_id?.trim() || '';
                    const linkMeta = widgetId ? widgetTestLinks[widgetId] : undefined;
                    const startTs = linkMeta?.startAt ? new Date(linkMeta.startAt).getTime() : 0;
                    const expiryTs = linkMeta?.expiresAt ? new Date(linkMeta.expiresAt).getTime() : 0;
                    const remainingMs = expiryTs ? expiryTs - nowMs : 0;
                    const isExpired = Boolean(expiryTs && remainingMs <= 0);
                    const windowMs = startTs && expiryTs && expiryTs > startTs ? (expiryTs - startTs) : DAY_MS;
                    const barValue = expiryTs ? Math.max(0, Math.min(100, (remainingMs / windowMs) * 100)) : 0;
                    ...
                    return (
                      ...
                      >
                        ...
                        <TableCell>
                          {linkMeta?.loading ? (
                            ...
                            <Stack spacing={0.6}>
                              ...
                              />
                              <Typography variant="caption" color="text.secondary">
                                Start: {linkMeta.startAt ? new Date(linkMeta.startAt).toLocaleString() : '-'}
                              </Typography>
                              <Typography variant="caption" color="text.secondary">
                                End: {new Date(linkMeta.expiresAt).toLocaleString()}
                              </Typography>
                            </Stack>
                          ) : (
                        ...
                        <TableCell sx={{ textAlign: 'right' }}>
                          <Stack direction="row" spacing={0.1} sx={{ justifyContent: 'flex-end', flexWrap: 'nowrap' }}>
                            <IconButton
                              size="small"
                              title="View"
                            ...
                            <IconButton
                              ...
                              disabled={!widgetId || Boolean(linkMeta?.loading)}
                              color="success"
                              sx={{ p: 0.45 }}
                            >
                ...
                <Stack spacing={1.8}>
                  <Box sx={accentPanelSx}>
                    ...
                    <TextField
                      fullWidth
                      size="small"
                      value={dialogTestLinkLoading ? 'Generating expiring test URL...' : dialogTestLink}
                      InputProps={{ readOnly: true }}
                      sx={fieldSx}
                    />
                    {dialogTestLinkExpiresAt && (
                      <Typography variant="caption" color="text.secondary" sx={{ mt: 0.8, display: 'block' }}>
                        This test link expires on {new Date(dialogTestLinkExpiresAt).toLocaleString()}.
                      </Typography>
                    )}
                    <Button
                      size="small"
...
<script src="https://your-domain.com/widget/dist/chatbot-widget.iife.js"><\/script>`}
      ...
      >
        <DialogTitle sx={{ fontWeight: 800 }}>Share Test Link by Email</DialogTitle>
        <DialogContent>
          <Stack spacing={1.3} sx={{ mt: 0.8 }}>
            <Typography variant="body2" color="text.secondary">
              Send a simple welcome email with secure test URL for {emailShareWidgetName || 'this agent'}.
            </Typography>
            <TextField
        ...
        <DialogActions>
          <Button onClick={handleCloseEmailDialog} disabled={emailShareSending}>Cancel</Button>
          <Button variant="contained" onClick={handleSendEmailShare} disabled={emailShareSending}>
